...
(function() {
  ...
  function resetPushBookkeeping(projects, categories, archive, trash) {
    ...
    // Notes: '' = UNKNOWN, not "pushed". The next push tick then compares
    // against the actual cloud (pushNotesIfNeeded reads it) instead of
    // assuming the local set already made it up — otherwise a note set that
    // failed to upload (e.g. rules rejected it) would never be retried until
    // edited. Adoption sites re-pin the hash right after adopting, so pulled
    // data is not immediately re-pushed.
    _lastPushedNotesHash = '';
  }
