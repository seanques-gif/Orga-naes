  ...
  document.addEventListener('keydown', (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
      ...
      saveModal.innerHTML = '<div style="background:var(--card,#12161c);border:1px solid var(--card-border,#232a34);border-radius:var(--radius-overlay);padding:24px;min-width:260px;text-align:center;color:var(--text,#d7dde5);">' +
        ...
        '<button id="_save-local" style="display:block;width:100%;padding:10px;margin-bottom:8px;border:none;border-radius:var(--radius-container);background:var(--accent,#2fd4ff);color:var(--accent-contrast,#04141b);font-size:var(--font-size-base);font-weight:600;cursor:pointer;">' + window._pf.pfIcon('save') + ' Save to Local (JSON)</button>' +
        '<button id="_save-cloud" style="display:block;width:100%;padding:10px;margin-bottom:8px;border:none;border-radius:var(--radius-container);background:var(--accent,#2fd4ff);color:var(--accent-contrast,#04141b);font-size:var(--font-size-base);font-weight:600;cursor:pointer;">' + window._pf.pfIcon('cloud') + ' Push to Cloud</button>' +
