...
## Phase B render-chunking (NEXT-PLAN, 2026-09-13)

B1 baseline + B2/B3/B4 implementation of NEXT-PLAN Phase B, measured on the
preview build via an in-app probe (performance.now() around each render stage;
preview longtask APIs are unavailable in the embedded context, so stage sums
stand in for long-task entries).

**Baseline (pre-chunk, warm steady state after one warmup render):**

| projects | render() total | list build | due panel |
|---|---|---|---|
| 42 | 10.3ms | 7.9ms | 1.2ms |
| 126 | 62.8ms | 56.6ms | 4.0ms |
| 250 | 156.2ms | 146.7ms | 6.7ms |

Cold-start at 42 measured 53.5ms, replicating the Phase 4 documented ~55ms
(that number was cold-cache; warm boards were already fast at this scale).
The list build is the entire scaling story: super-linear growth (×10 cost for
×6 data), one unsplittable 147ms task at 250 projects (worst observed 197ms).
Detail pane is negligible at every scale (0.1–0.6ms).

**After (`0b4a4af`):** category labels render synchronously; list *items*
append in requestAnimationFrame slices under a 6ms budget, engaged only above
60 visible projects. Sync render() cost at 250: **16.0ms** (first) / 25.8ms
(warm) — ~85% cut; remaining work streams in across 8 observed slices, never
blocking input. A monotonically increasing token cancels superseded chains:
a burst of 3 renders yields exactly one complete list (188 rows = 250 minus
62 completed-filtered — the exact filter math), which also bounds the
per-keystroke cost of search re-renders. Small boards are untouched: 42
projects measured 15.0ms on the sync path (was 14.8ms, within noise).

**Measurement caveats recorded honestly:** the embedded preview throttles
rAF to ~2.5fps when unfocused, so *wall-clock* settle time there (3.1s) is
not representative — foreground tabs run continuous frames (~16.7ms/slice).
The 6ms budget + token design holds under both cadences; interaction latency
(the INP-relevant number) is what improved, and it is cadence-independent.


## Font system day (2026-09-13) — picker, custom fonts, embedding, trim, Inter default

**Motivation:** owner liked the iOS look and asked for it on desktop; investigation showed the iPhone renders the same IBM Plex at ~11px on a dense screen, i.e. a *size* perception — but the actual iOS face is SF Pro, and the closest free lookalike is Inter. That spun into a full typography arc across five commits, all verified live in the preview and green in the suite.

**1. Font picker (61b8faf).** Appearance section gained a Font select applied through the `--font-sans` token with per-device persistence (same safeGet/safeSet pattern as the size/scale sliders).

**2. Custom Google Fonts (2a0371b).** A "Custom Google Font..." option reveals an input; any family name loads on demand. Three real bugs found by live probing, each instructive:
- `document.fonts.check()` returns **true for nonexistent families** (nothing pending reads as available). Replaced with the css2 API as existence oracle: HTTP 200 = family exists, 400 = not. Unspoofable.
- css2 rejects the **whole request** if any requested weight is out of range (Lora starts at 400; my 300-weight ask killed it). Fixed with a weight ladder: `:wght@400;500;600;700` → `:wght@400;700` → bare family.
- Chromium **never fires `onload`** when a stylesheet is re-pointed at an already-cached URL (proved: bytes transferred, zero events, timeout-only exit). Fix: poll `link.sheet` alongside the events.

**3. Embedded fonts (72e4152).** IBM Plex Sans 400-700, Plex Mono 400-600, Inter 400-700 — latin subsets, base64 woff2, generated by `scripts/gen-embedded-fonts.mjs` into `src/styles/00-embedded-fonts.css` (loads first in order.json). Boot font links removed from the head; preconnects stay for the custom-font path. CSP `font-src` gained `data:`; csp.test expects it. Verified: **zero requests** to fonts.googleapis/gstatic at boot, faces register from the data URIs, custom-font path still works. Artifact: 683,609 → **1,249,508 B** (+553 KB for 11 faces).

**4. Trim to actual usage (2b8069e).** Weight audit across CSS+JS: sans uses 400/500/600/700 everywhere; mono only 400/600 → **Mono 500 dropped**. Two phantom literals found and fixed (`font-weight: 300` on the FAB and `800` on week day numbers rendered as 400/700 via font matching all along). Artifact: **1,229,268 B** (−20,240 B). Pixels unchanged.

**5. Inter as shipped default (bdfe608).** Token default is now Inter-first; picker reads "Inter (default, iOS-like)" / "Console classic (IBM Plex)" / System / Custom. Legacy saved `'inter'` values (opt-in era) normalize to '' on load. Existing explicit prefs untouched. SW cache bumped per change (…0003/0004/0005) — the ask-before-update pill was exercised on real bumps twice and worked (waits styled top-right, applies on click).

**End state:** iOS-clean typography out of the box on every device, fully offline, exact-usage bundle, three font escape hatches (Plex classic, system stack, any Google Font) — all enforced by 291 automated checks.
