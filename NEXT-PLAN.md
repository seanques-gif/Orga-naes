...
## Phase C — Security backlog ✅ COMPLETE (2026-09-13). C1 rules published + stranger-test verified; C2 CSP hash-pinned; C3 SRI pinned; C4 ledger closed. SEC-03/04/05/06/08 all addressed.
- C1 ✅ **RTDB `.validate` rules (SEC-03/06) — DONE 2026-09-13.** Strict generated rules (scripts/gen-rtdb-rules.mjs -> database.rules.json, 75-check shadow suite wired into npm test). Owner rehearsal on scratch project all green (sign-in, CRUD, notes sync, incognito Permission denied). Production export replay caught 5 unmodeled real fields (project color/description/dueAt, subtask timeLogged/timerStart) BEFORE publish — generator extended, export replay is now a permanent suite gate (f6e5bb5). Production rules published 2026-09-13; owner confirmed incognito `users.json` -> `{"error":"Permission denied"}` on the REAL database. C1 closed.
...
- C3 ✅ **SRI (SEC-05/08) — DONE 2026-09-13.** sha384 `integrity` + `crossorigin=anonymous` on all three gstatic bundles (hashes computed from Google's served bytes, cross-verified by two independent tools). Live boot verified under enforcement (all bundles execute, zero violations). Gate lives in tests/csp.test.mjs (25 checks): any CDN script missing sha384 integrity fails npm test. Version-bump procedure: run `node scripts/gen-sri.mjs <new-version>`, paste the printed tags into src/template.html, rebuild, commit.
